...
class ExecCommand(BaseCommand):
    ...
    Examples:
        # Use active context (set by awsctl switch)
        awsctl exec -- terraform plan

        # Explicit org without changing shell context (safe for scripts)
        awsctl exec --org prod -- aws s3 ls
        awsctl exec --org fdr-gvc --account 111111111111 --role ReadOnly -- terraform show
    """
    ...
    def execute(self, args) -> int:
        ...
        if not org_name:
            self.console.print(
                ...
                "Run [bold]awsctl switch <org>[/bold] first, or use: "
                "[bold]awsctl exec --org <org> -- <command>[/bold]"
            )
            return 1
