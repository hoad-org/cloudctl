"""
awsctl prompt — emit a formatted string for shell prompt integration.

Usage:
    awsctl prompt              # plain text: ☁ bt-avm (123456789012/AdminAccess/us-east-1)
    awsctl prompt --short      # short form: ☁ bt-avm
    awsctl prompt --json       # JSON for custom tooling
    awsctl prompt --format ps1 # PS1-safe escape sequences (bash/zsh)
    awsctl prompt --starship   # print a starship.toml snippet and exit
    awsctl prompt --p10k       # print a p10k zsh segment snippet and exit
...
Add to your shell:
  ...
  # Starship — add to ~/.config/starship.toml  (awsctl prompt --starship)

  # Powerlevel10k — add to ~/.p10k.zsh  (awsctl prompt --p10k)
"""
...
[custom.awsctl]
command = "awsctl prompt --short 2>/dev/null"
when = "awsctl prompt --json 2>/dev/null | python3 -c \\"import sys,json; d=json.load(sys.stdin); exit(0 if d.get('org') else 1)\\""
format = "[$output]($style) "
style = "bold yellow"
shell = ["sh", "-c"]
description = "Current awsctl cloud context"

# ── Optional: show in the right side of the prompt ────────────────────────
# [right_format]
# format = "$custom.awsctl"
""")


def _print_p10k_snippet() -> None:
    sys.stdout.write("""\
# ── awsctl Powerlevel10k integration ──────────────────────────────────────
# Add this block to ~/.p10k.zsh
#
# 1. Paste the function below into ~/.p10k.zsh
# 2. Add 'awsctl' to POWERLEVEL9K_LEFT_PROMPT_ELEMENTS or RIGHT
#    e.g.:  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(... awsctl)

function prompt_awsctl() {
  local ctx
  ctx=$(awsctl prompt --short 2>/dev/null) || return
  [[ -z "$ctx" ]] && return
  p10k segment -f yellow -i '☁' -t "$ctx"
}

function instant_prompt_awsctl() {
  # Instant prompt: show last known context from cache without running awsctl
  local ctx_file="${XDG_CONFIG_HOME:-$HOME/.config}/awsctl/current_context.json"
  [[ -f "$ctx_file" ]] || return
  local org
