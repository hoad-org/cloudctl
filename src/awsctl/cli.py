...
from awsctl.utils import console, debug_print, open_browser, set_debug
...
def _resolve_account_id(ref: OrgRef, target: Optional[str]) -> str:
    ...
    for a in accts:
        if a.account_name.lower() == target_lower:
            # [FIX] Cast to str to satisfy Mypy strictness if inference fails
...
def cmd_login(args: argparse.Namespace) -> int:
    ...
    if not org_name:
        try:
            ...
            debug_print(f"Error autoloading config for login: {e}")
    ...
    if account or role:
        if account:
            try:
                ...
                debug_print(f"Account resolution warning: {e}. Trying raw ID.")
                args.target = account
    ...
    try:
        ...
        if not region:
            try:
                ...
                debug_print(f"Error fetching org default region: {e}")
                region = "us-east-1"
...
def cmd_exec(args: argparse.Namespace) -> int:
    ...
    if not region and current_org:
        try:
            ...
            debug_print(f"Context load warning: {e}")
    ...
    if account:
        try:
            ...
            debug_print(f"Exec account resolution warning: {e}")
