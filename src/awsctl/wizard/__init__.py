"""
awsctl.wizard — interactive setup wizard for first-time configuration.
...
Also called by:  awsctl org add  (provider-specific paths only)
"""
...
def _show_welcome() -> None:
    ...
    try:
        ...
        utils.console.print(
            Panel(
                Align.center(
                    f"[bold cyan]awsctl v{ver}[/bold cyan]\n"
        ...
        utils.console.print(f"\n[bold cyan]awsctl v{ver} — Setup Wizard[/bold cyan]\n")
    ...
    utils.console.print(
        "  [dim]Add more organisations any time with: awsctl org add[/dim]"
    )
    ...
    try:
        ...
        if any(o.get("provider", "aws") == "aws" for o in orgs):
            if core.cmd_config_sync() != 0:
                _warn("AWS profile sync had issues — run 'awsctl doctor' to diagnose.")
...
def _install_shell_integration() -> None:
    ...
    if confirmed:
        ...
        _warn(
            "Skipped. Install manually later with:  awsctl init --shell-only"
        )
...
def _show_next_steps(orgs: List[Dict[str, Any]]) -> None:
    ...
    for org in orgs:
        utils.console.print(f"       awsctl login [bold]{org['name']}[/bold]")
    utils.console.print()
    utils.console.print("  [bold]3. Switch context and verify:[/bold]")
    first = orgs[0]["name"] if orgs else "<org>"
    utils.console.print(f"       awsctl switch [bold]{first}[/bold]")
    utils.console.print(f"       awsctl env")
    utils.console.print()
    utils.console.print(
        "  [dim]awsctl doctor[/dim]          — run health checks at any time\n"
        "  [dim]awsctl org add[/dim]         — add more organisations\n"
        "  [dim]awsctl org list[/dim]        — list configured organisations\n"
        "  [dim]awsctl --help[/dim]          — full command reference\n"
    )
