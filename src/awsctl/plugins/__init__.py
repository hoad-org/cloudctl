...
def load_plugins(enabled: Optional[Iterable[str]]) -> list[object]:
    ...
    for name in enabled or []:
        # [FIX] Logic Defect: Removed hardcoded 'myorg' namespace.
        # Now strictly enforces the package namespace for security.
        if not name.startswith("awsctl.plugins."):
            console.print(
                f"[bold red][SECURITY] Blocked illegal plugin load attempt: {name}[/]"
            )
            sys.exit(1)
        ...
        try:
            ...
            console.print(
                f"[bold red][ERROR] Critical: Failed to load required plugin '{name}': {e}[/]"
            )
            sys.exit(1)
    return mods
        ...
        if any(
            ...
            try:
                ...
                try:
                    ...
                    console.print(
                        "[yellow][WARNING] Plugin hook signature mismatch. Skipping.[/]"
                    )
    except SystemExit:
        # Propagate SystemExit so it can be caught by future.result()
        raise
        ...
        for m in mods:
            ...
            if callable(fn):
                ...
                try:
                    ...
                    # Propagate SystemExit from thread to main process
                    sys.exit(1)
    finally:
