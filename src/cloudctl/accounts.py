...
# Re-export: tests monkeypatch cloudctl.accounts.load_active_sso_token
...
def list_accounts(org_data: Any) -> List[Account]:
    ...
    source so that existing tests can monkeypatch cloudctl.accounts.load_active_sso_token
...
def get_account_list(org_data: Any, force_sync: bool = False) -> List[Dict[str, str]]:
    ...
    Used by AccountsCommand (cloudctl accounts <org>).
    """
