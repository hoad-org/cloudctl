...
def _emit_eval_exports(export_str: str) -> None:
    """Emit shell export statements to stdout for EVAL mode.

    This is intentional: the shell wrapper (bash/zsh/fish) captures this
    output and sources it to inject credentials into the current shell session.

    SECURITY: Credentials are NOT persisted to disk or logs:
    - Output goes to stdout only (captured by shell wrapper in-memory)
    - Shell wrapper sources output into current shell process
    - Ephemeral credentials exist only in current shell session
    - No disk I/O, no log persistence, no credential storage

    CodeQL note: This function is flagged for logging credentials, which is
    intentional and safe. The credentials are temporary session tokens that
    exist only in the invoking shell's memory and are never persisted.
    """
    # noqa: S324 # nosec: Intentional eval-mode credential output to transient stdout
    _safe_emit_to_stdout(export_str)


def _safe_emit_to_stdout(output: str) -> None:
    """Emit output to stdout for shell capture.

    This is only used for ephemeral session tokens in EVAL mode.
    """
    print(output)  # lgtm [py/clear-text-logging-sensitive-data]


def _safe_emit_to_stderr(output: str) -> None:
    """Emit formatted error output to stderr.

    Input is pre-sanitized by format_error() which filters sensitive keys.
    """
...
def cmd_switch(args: Any) -> int:
    ...
    from .guardrails import validate_role_access

    try:
        target = (
            getattr(args, "target", None)
            or getattr(args, "org", None)
            or getattr(args, "org_flag", None)
        )
        ...
        from . import role_validator
        from .sso_cache import load_active_sso_token, OrgRef

        try:
            # Load token to validate role
            org_ref = OrgRef(
                name=org_data.get("name", ""),
                sso_start_url=org_data.get("sso_start_url", ""),
                sso_region=org_data.get("sso_region", ""),
            )
            ...
            if token:
                ...
                if not is_valid:
                    if non_interactive:
                        # In non-interactive mode, show error and available roles
                        ...
                        # In interactive mode, attempt to correct the role selection
            ...
            # If role validation fails, log warning but continue
...
def cmd_cache_clear(args: Any) -> int:
    """Clear cached AWS CLI credentials and SSO tokens (forces re-login)."""
    core.cmd_cache_clear()
    console.print("[green]✓[/] Cleared cached credentials and SSO tokens.")
    return 0
...
def _upgrade_via_github(args: Any) -> int:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".whl", delete=False, mode="wb") as f:
            # NamedTemporaryFile created with 0o600 by default (Python 3.11+)
            # Explicitly set mode for compatibility with older Python versions
            f.write(wheel_data)
            tmp_path = f.name
            # Ensure secure permissions for downloaded wheel file
...
def cmd_list(args: Any = None) -> int:
    """List organizations (shortcut for 'org list')."""
...
def _build_parser():
    ...
    for name in ("switch", "use"):
        ...
        sp.add_argument(
            "--non-interactive",
            action="store_true",
            help="Skip interactive prompts; require all arguments (for automation/Claude)",
        )
    ...
    ap.add_argument("org", nargs="?", help="Organization name")
    ap.add_argument(
        "--org",
        dest="org_flag",
        help="Organization name (flag form; same as positional)",
    ...
    list_p = org_sub.add_parser("list", help="List configured organizations")
    list_p.add_argument(
        "--format",
        choices=["table", "json"],
        default="table",
...
def main(argv: Optional[List[str]] = None) -> int:
    try:
        ...
        # matching: password, token, secret, key, credential, api_key, etc.
