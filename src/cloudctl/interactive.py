...
def _org_data_to_orgref(org_data: Dict[str, Any]) -> OrgRef:
    """Convert org config dict to OrgRef object for SSO token loading."""
    return OrgRef(
...
def _run(
    ...
    token = load_active_sso_token(_org_data_to_orgref(org_data))
    if not token:
