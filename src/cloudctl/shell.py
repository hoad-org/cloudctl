...
AWSCTL_WRAPPER = """# CLOUDCTL SHELL INTEGRATION
cloudctl() {
...
AWSCTL_PS_WRAPPER = r"""# CLOUDCTL SHELL INTEGRATION
function cloudctl {
    ...
    if ($mutating -contains $first -or $loginWithFlags) {
        ...
        try {
            $cloudctlBin = (Get-Command cloudctl -CommandType Application -ErrorAction Stop).Source
            $env:AWSCTL_WRAPPER_ACTIVE = '1'
            & $cloudctlBin --eval @args | Out-File -FilePath $tmp -Encoding utf8
...
AWSCTL_FISH_WRAPPER = r"""# CLOUDCTL SHELL INTEGRATION
function cloudctl
    ...
    if test $needs_eval -eq 1
        set -l tmp (mktemp)
        AWSCTL_WRAPPER_ACTIVE=1 command cloudctl --eval $argv > $tmp
        set -l ec $status
        if test $ec -eq 0
            while read -l line
...
def detect_fish_function_file() -> Path:
    """Return the fish function file path for the cloudctl function."""
    config_fish = os.environ.get("XDG_CONFIG_HOME", str(Path.home() / ".config"))
    return Path(config_fish) / "fish" / "functions" / "cloudctl.fish"
...
def inject_shell_function(path: Path) -> bool:
    ...
    # Idempotence Check.
    if "CLOUDCTL SHELL INTEGRATION" in content:
        return False
    ...
    try:
        ...
        # Preserve original file permissions only if they're secure (user-readable)
        # Otherwise use secure defaults (user-readable, group/other not readable)
        if path.exists():
            st = path.stat()
            # Only copy permissions if file is user-readable and not world/group readable
            if (st.st_mode & 0o077) == 0:  # No group/other bits set
                os.chmod(temp_path, st.st_mode)
            else:
                # Secure default: user read/write
                os.chmod(temp_path, 0o600)
        else:
            # Secure default for new files: user read/write only
            os.chmod(temp_path, 0o600)
...
def remove_powershell_function(path: Path) -> bool:
    ...
    try:
        content = path.read_text(encoding="utf-8")
        if "CLOUDCTL SHELL INTEGRATION" not in content:
            return False
...
def inject_fish_function(path: Path) -> bool:
    try:
        if path.exists() and "CLOUDCTL SHELL INTEGRATION" in path.read_text(
            encoding="utf-8"
        ):
            return False
        path.parent.mkdir(parents=True, exist_ok=True)
...
def remove_fish_function(path: Path) -> bool:
    if not path.exists():
        return False
    try:
        if "CLOUDCTL SHELL INTEGRATION" not in path.read_text(encoding="utf-8"):
            return False
        path.unlink()
        return True
    except Exception:
