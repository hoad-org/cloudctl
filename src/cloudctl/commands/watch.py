"""
cloudctl watch — keep credentials alive by auto-refreshing before expiry.

Usage:
    cloudctl watch [org]             # refresh active (or named) org every check-interval
    cloudctl watch bt-avm --interval 300   # custom check interval in seconds
    ...
    def execute(self, args) -> int:
        ...
        if not org_name:
            self.console.print(
                ...
                "Run [bold]cloudctl switch <org>[/bold] first, or pass org name: "
                "[bold]cloudctl watch <org>[/bold]"
            )
            return 1
        ...
        self.console.print(
            f"[bold cyan]cloudctl watch[/bold cyan]  "
