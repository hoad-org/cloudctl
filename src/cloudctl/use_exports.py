...
def get_credentials(
    account: str, role: str, region: str, org_ref: Optional[Any] = None
) -> Dict[str, str]:
    # Load active SSO token from cache — required for get-role-credentials
    # If org_ref provided, use it; otherwise try to load from current context or use minimal ref
    if org_ref is None:
        org_ref = OrgRef(name="", sso_start_url="", sso_region=region)
...
def emit_exports(org: Any, account: str, role: str, region: str) -> str:
    ...
    monkeypatch cloudctl.use_exports._aws_json remain valid.
    """
    ...
    org_ref = (
        org
        if isinstance(org, OrgRef)
        else OrgRef(
            name=(
                org.get("name", "")
                if isinstance(org, dict)
                else getattr(org, "name", "")
            ),
            sso_start_url=(
                org.get("sso_start_url", "")
                if isinstance(org, dict)
                else getattr(org, "sso_start_url", "")
            ),
            sso_region=(
                org.get("sso_region", "")
                if isinstance(org, dict)
                else getattr(org, "sso_region", "")
            ),
        )
    )
    if not load_active_sso_token(org_ref):
