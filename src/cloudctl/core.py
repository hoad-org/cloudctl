...
def load_orgs_config():
    """
    Load orgs config — patchable at cloudctl.core.load_orgs_config.

    Calls config.load_orgs_config dynamically so that patches on
    cloudctl.config.load_orgs_config are also respected.
    """
...
def cmd_exec(account: str, role: str, region: str, command: List[str]) -> int:
    ...
    if account is not None and role is not None and org.get("provider", "aws") == "aws":
        try:
            # BUG #4 FIX: Pass org_ref to get_credentials so it can find cached SSO token
            from .sso_cache import OrgRef
            ...
            creds = _ue.get_credentials(_account, _role, _region, org_ref)
            env = os.environ.copy()
            env.update(creds)
            try:
...
def cmd_env() -> str:
    ...
    ctx = context_manager.load_context()
    if not ctx:
        return "No active context. Run 'cloudctl switch' first."
