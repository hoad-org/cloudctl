...
def sso_list_accounts(token: Any, region: Optional[str] = None) -> List[Dict[str, str]]:
    tk = token.accessToken if hasattr(token, "accessToken") else token

    # Validate token is not expired (BUG #2 fix)
    if hasattr(token, "expiresAt"):
        try:
            expires = datetime.fromisoformat(token.expiresAt.replace("Z", "+00:00"))
            if expires < datetime.now(expires.tzinfo):
                raise RuntimeError(
                    f"SSO token expired at {token.expiresAt}. "
                    f"Run 'cloudctl login' to refresh."
                )
        except Exception as e:
            if "expired" in str(e).lower():
                raise
            # If we can't parse expiry, continue and let AWS validate
...
def sso_list_account_roles(
    token: Any, account_id: str, region: Optional[str] = None
) -> List[Dict[str, str]]:
    tk = token.accessToken if hasattr(token, "accessToken") else token

    # Validate token is not expired (same as sso_list_accounts)
    if hasattr(token, "expiresAt"):
        try:
            expires = datetime.fromisoformat(token.expiresAt.replace("Z", "+00:00"))
            if expires < datetime.now(expires.tzinfo):
    ...
    # Ensure region is provided and valid (BUG #3 fix)
    if not region:
        raise ValueError("Region is required for sso_list_account_roles")
