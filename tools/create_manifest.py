...
def create_manifest() -> None:
    ...
    for root_path, dirs, files in os.walk(repo_root):
        ...
        for file in sorted(files):
            ...
            # Inclusion criteria
            is_valid_ext = file_path.suffix in TEXT_EXTENSIONS
            is_valid_name = file_path.name in {"Makefile", "Dockerfile", ".gitignore"}

            if not (is_valid_ext or is_valid_name):
                ignored_extensions[file_path.suffix or "no_extension"] += 1
                continue
    ...
    header_json: dict[str, Any] = {
        ...
        "ignored_stats": dict(ignored_extensions),
        "files": manifest_entries,
    }

    # Assemble Final Content
    manifest_body: str = (
        f"# 📦 Repo Holistic Snapshot (Redacted)\n"
        f"Generated: {header_json['generated']}\n"
        "---MANIFEST-START---\n"
        f"{json.dumps(header_json, indent=2)}\n"
        "---MANIFEST-END---\n\n" + "".join(file_blocks)
    )
    ...
    print(
        f"📊 Included: {len(manifest_entries)} files | Ignored: {sum(ignored_extensions.values())} files"
    )
    ...
    if ignored_extensions:
        ...
        print(
            "💡 Hint: Add extensions to TEXT_EXTENSIONS in the script if they are missing."
        )
