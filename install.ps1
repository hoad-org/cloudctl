# install.ps1 — cloudctl installer for Windows PowerShell / pwsh
...
$GithubRepo = "aws-terraform-infra-cloudops-cloudctl"
$ApiBase    = "https://api.github.com/repos/$GithubOrg/$GithubRepo"

Write-Host "🚀 Starting cloudctl installation..." -ForegroundColor Cyan
...
if (-not $PythonBin) {
    Write-Host "❌ cloudctl requires Python 3.12 or newer." -ForegroundColor Red
...
Write-Host "     cloudctl --version"
Write-Host "     cloudctl doctor"
Write-Host ""
Write-Host "   To upgrade later:  cloudctl upgrade   (requires GITHUB_TOKEN)" -ForegroundColor White
