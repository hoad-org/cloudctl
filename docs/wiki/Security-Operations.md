...
This document defines the **operational procedures** for managing, auditing, and responding to incidents within the `awsctl` ecosystem. It outlines how to maintain the security posture defined in the [Security Overview](./security-overview.md) during day-to-day use.

---

## 🧾 Audit and Observability

`awsctl` operations are designed to be transparent and verifiable through native cloud logging.
...
* **Automation:** Security teams should monitor for `AssumeRole` events where the `userAgent` matches `awsctl/*`.



### 2. Local Forensic Artifacts
While `awsctl` does not persist credentials, it does maintain a minimal, non-sensitive execution log for troubleshooting:
* **Location:** `~/.config/awsctl/logs/` (or OS equivalent).
...
The **Policy Registry** is the operational control point for `awsctl` behavior.
...
To immediately block a specific role or account from being accessed via `awsctl`:
1. Update the central registry to move the target to a `deny_list`.
2. Push the update.
3. `awsctl` clients will enforce the block upon their next registry sync (typically every 60 minutes or on force-refresh).
...
Since `awsctl` uses STS temporary credentials, access can be revoked globally via AWS:
...
* **Effect:** `awsctl` will fail to refresh or broker new credentials immediately.
...
The registry can enforce a `min_version` for the `awsctl` binary. 
* **Action:** If a security vulnerability is found in an older version of `awsctl`, update the `min_version` in the registry.
* **Effect:** All older clients will refuse to execute until the user updates.

### 2. Secret Scanning
`awsctl` developers and operators must ensure that:
...
Security Operations for `awsctl` focus on **centralized policy** and **decentralized execution**. By leveraging native AWS and IdP controls, operators can manage human access at scale without managing local state.
