...
This document provides a **high-level security overview** of `awsctl`. It is intended for Security Engineers, Platform Owners, Auditors, and Risk Reviewers.
...
`awsctl` is a client-side identity orchestration tool designed to interact with AWS securely without introducing new control planes, long-lived credentials, or implicit authority.
...
If `awsctl` is removed, the organization remains secure and functional.

---

## 🔎 What awsctl Is (Security Perspective)
...
`awsctl` sits between intent and execution, enforcing policy at runtime.
...
### Identity Source
`awsctl` relies on external systems for authentication:
* **AWS IAM Identity Center**
* **External IdPs** (e.g. Okta, Entra ID)

### Security Properties
* **MFA:** Enforced outside `awsctl` at the IdP level.
* **Secrets:** No passwords or authentication secrets are handled or stored by `awsctl`.
* **Consumption:** `awsctl` only consumes proof of identity, never the credentials themselves.
...
`awsctl` does not grant permissions. It only selects from what already exists within the AWS control plane.
...
All AWS credentials used by `awsctl` are:
...
### Explicit Guarantees
`awsctl` will never generate access keys, store credentials on disk, or share credentials between processes.
...
### Execution Model
`awsctl` executes in a single-invocation, ephemeral context. This ensures no background agents, no hidden state, and clear forensic traceability.
...
* **One-way output:** `awsctl` → shell.
* **Validation:** Strict character allow-lists for output.
* **No execution:** `awsctl` never executes shell commands directly.
...
### Audit & Logging
`awsctl` relies on native audit systems:
* **AWS CloudTrail:** For all API activity.
* **IdP Logs:** For authentication events.
* **Local Logs:** Structured output for local traceability.

### Failure Mode
`awsctl` fails closed, loudly, and visibly. Ambiguity, partial success, or silent fallbacks are treated as failures.
...
1.  `awsctl` holds no long-lived secrets.
...
> [!IMPORTANT]
> `awsctl` is secure not because it is powerful, but because it refuses to become powerful.