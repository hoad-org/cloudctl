...
* **In-Memory Only:** `awsctl` never writes long-term credentials (`AWS_SECRET_ACCESS_KEY`) to disk. They are held in process memory and exported directly to the shell environment via the "Context Bridge".
...
To support corporate inspection proxies (Zscaler, Netskope), `awsctl` (via Python's `requests`) must trust the system certificate store.
...
* The Public Key is pinned in the binary (`src/awsctl/registry.py`).
