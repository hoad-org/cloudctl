"""
tests/test_switch_back.py — Unit tests for `cloudctl switch -` (previous context restore).
...
class TestSwitchBack:
    ...
    def test_switch_dash_prints_success_message(self, capsys):
        with patch("cloudctl.context_manager.get_previous_context", return_value=_PREV):
            with patch("cloudctl.config.get_org", return_value=_ORG_DATA):
                with patch("cloudctl.cli.emit_exports", return_value=""):
...
class TestSwitchBackErrors:
    def test_no_previous_context_returns_1(self, capsys):
        with patch("cloudctl.context_manager.get_previous_context", return_value=None):
            with patch.object(cli, "console", MagicMock()):
                rc = cli.cmd_switch(_make_args())
