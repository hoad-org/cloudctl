...
def test_wrapper_injection(mock_home):
    ...
    assert "CLOUDCTL SHELL INTEGRATION" in content
...
def test_shell_injection_read_fail(monkeypatch, tmp_path):
    ...
    assert "CLOUDCTL SHELL INTEGRATION" in rc.read_text(encoding="utf-8")
...
def test_inject_shell_newline_insertion(tmp_path):
    ...
    assert "existing_content\n\n\n# CLOUDCTL SHELL INTEGRATION" in content


@pytest.mark.skipif(os.name == "nt", reason="Permission preservation varies on Windows")
def test_inject_shell_hardens_insecure_permissions(tmp_path, monkeypatch):
    """An rc file with group/other bits (insecure) is hardened to 0o600, not preserved."""
    rc = tmp_path / "rc_perms"
    rc.touch()

    mock_stat = MagicMock()
    mock_stat.st_mode = 0o777  # world-writable — must NOT be preserved
    with patch.object(Path, "stat", return_value=mock_stat):
        with patch("os.chmod") as mock_chmod:
            shell.inject_shell_function(rc)
            assert mock_chmod.called
            # Secured down to owner-only, not the insecure 0o777.
            assert mock_chmod.call_args[0][1] == 0o600


def test_inject_shell_preserves_already_secure_permissions(tmp_path, monkeypatch):
    """An rc file already owner-only (no group/other bits) keeps its mode."""
    rc = tmp_path / "rc_perms_secure"
    rc.touch()

    mock_stat = MagicMock()
    mock_stat.st_mode = 0o600  # already secure
    with patch.object(Path, "stat", return_value=mock_stat):
        with patch("os.chmod") as mock_chmod:
            shell.inject_shell_function(rc)
            assert mock_chmod.called
