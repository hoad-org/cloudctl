...
def test_cmd_exec_subprocess_fail(mock_rich_console, monkeypatch):
    ...
    # cmd_exec resolves credentials via use_exports.get_credentials, then execvpe;
    # stub it so we reach the execvpe call and exercise the not-found (127) path.
    monkeypatch.setattr(
        "cloudctl.use_exports.get_credentials",
        lambda *a, **k: {
