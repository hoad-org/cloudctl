...
cloudctl() {
    ...
    if [[ $needs_eval -eq 1 ]]; then
        local tmp exit_code
        tmp=$(mktemp)
        CLOUDCTL_WRAPPER_ACTIVE=1 command cloudctl --eval "$@" > "$tmp"
        exit_code=$?
        rm -f "$tmp"
        return $exit_code
    else
        CLOUDCTL_WRAPPER_ACTIVE=1 command cloudctl "$@"
    fi
}
"""
...
class TestRemoveCloudctlBlocks:
    MARKERS = ["# CLOUDCTL SHELL INTEGRATION", "# cloudctl completion"]
    ...
    def test_removes_full_wrapper_function(self):
        ...
        assert "CLOUDCTL SHELL INTEGRATION" not in joined
    ...
    def test_dollar_brace_in_function_not_confused_with_block_end(self):
        ...
        assert "CLOUDCTL_WRAPPER_ACTIVE" not in joined
            ...
            with patch.object(cli, "console", MagicMock()):
                with patch(
                    ...
                    with patch("InquirerPy.inquirer.confirm") as mock_confirm:
                        ...
                        cli.cmd_uninstall(self._make_args())

        content = profile.read_text()
        assert "awsctl()" not in content
        assert "CLOUDCTL SHELL INTEGRATION" not in content
    ...
    def test_keep_config_preserves_config_dir(self, tmp_path):
        ...
        with patch(
            ...
            side_effect=lambda p: (
                str(config_dir) if "config/awsctl" in p else "/nonexistent"
            ),
        ):
            with patch.object(cli, "console", MagicMock()):
                with patch(
                    ...
                    with patch("InquirerPy.inquirer.confirm") as mock_confirm:
                        mock_confirm.return_value.execute.return_value = True
                        cli.cmd_uninstall(self._make_args(keep_config=True))
        ...
        with patch(
            ...
            with patch.object(cli, "console", MagicMock()):
                with patch(
                    ...
                    with patch("InquirerPy.inquirer.confirm") as mock_confirm:
                        mock_confirm.return_value.execute.return_value = True
                        cli.cmd_uninstall(self._make_args(package_only=True))
