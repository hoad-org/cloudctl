# uninstall.ps1 — cloudctl clean uninstaller for Windows PowerShell / pwsh
# Run from the repo root:  .\uninstall.ps1
#Requires -Version 5.1
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

Write-Host "🗑️  Starting cloudctl uninstallation..." -ForegroundColor Cyan
...
pip uninstall -y cloudctl 2>$null
...
$cloudctlDir = Join-Path $env:USERPROFILE ".cloudctl"
if (Test-Path $cloudctlDir) {
    Remove-Item -Recurse -Force $cloudctlDir
    Write-Host "  ✓ Removed $cloudctlDir" -ForegroundColor Green
}

$ctxFile = Join-Path $env:APPDATA "cloudctl\current_context.json"
